<table class="form-table font-item">
  <thead>
    <tr>
      <th colspan="2">
        <?php esc_html_e("Font", 'greenshift') . ' #' . $i; ?>
      </th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td class="label-td">
        <label for="font_family_name_<?php echo esc_attr($i)?>">
          <?php esc_html_e("Font Family Name", 'greenshift'); ?>
        </label>
      </td>
      <td>
        ...
        <div class="input-desc">
          <?php esc_html_e("Make sure that name has only Latin symbols without numbers and special characters", 'greenshift'); ?>
        </div>
      </td>
    </tr>
      ...
      <tr>
        <td class="label-td">
          <label for="font_<?php echo esc_attr($ext)?>_file_<?php echo $i?>">
            <?php echo '.' . $ext . ' ' . esc_html__("Font file", 'greenshift'); ?>
          </label>
        </td>
        <td>
          <input
          ...
          <div class="input-desc">
            <?php echo sprintf(__("Upload .%s font file", 'greenshift'), $ext); ?>
          </div>
          <?php if(!empty($options[$ext])):?>
            <div class="current-file" style="margin-top: 5px;">
              <?php esc_html_e('Current file', 'greenshift')?>: <b><?php echo basename($options[$ext])?></b>

              <div class="current-file-actions">
                (<a href="<?php echo esc_url($options[$ext])?>"><?php esc_html_e('Download', 'greenshift')?></a>
                /
                <a href="#" class="remove-font-file"><?php esc_html_e('Remove', 'greenshift')?></a>)
              </div>
            </div>
          <?php endif;?>
        </td>
